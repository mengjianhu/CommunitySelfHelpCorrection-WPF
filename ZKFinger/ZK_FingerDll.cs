   ...
   public class ZK_FingerDll
    {
        ...
        /// <returns>成功返回1，否则返回错误代码</returns>
        [DllImport("ID_FprCap.dll", CallingConvention = CallingConvention.StdCall)]
        public extern static int LIVESCAN_GetDesc(IntPtr pszDesc);
        /// <summary>
        /// 获取采集接口错误信息
        /// </summary>
        ...
        /// <returns>成功返回1，否则返回错误代码</returns>
        [DllImport("ID_FprCap.dll", CallingConvention = CallingConvention.StdCall)]
        public extern static int LIVESCAN_GetErrorInfo(int nErrorNo, char[] pszErrorInfo);
        /// <summary>
        /// 设置存放采集数据的内存为空
        /// </summary>
        ...
        /// <returns>成功返回1，否则返回错误代码</returns>
        [DllImport("ID_Fpr.dll", CallingConvention = CallingConvention.StdCall)]
        unsafe public extern static int FP_FeatureMatch(byte[] pFeatureData1, byte[] pFeatureData2, out float pfSimilarity);
        /// <summary>
        /// 指纹图像数据与指纹特征数据比对
        /// </summary>
        ...
        /// <returns>成功返回1，否则返回错误代码</returns>
        [DllImport("ID_Fpr.dll", CallingConvention = CallingConvention.StdCall)]
        unsafe public extern static int FP_ImageMatch(byte[] pFingerImgBuf, byte[] pFeatureData, out float pfSimilarity);
        /// <summary>
        /// 指纹图像数据压缩
        /// </summary>
        /// <param name="cScannerType">指纹采集器代码</param>
